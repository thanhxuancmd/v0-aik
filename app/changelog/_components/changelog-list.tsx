  ...
  return (
    <div className="flex w-full flex-col">
      {changelogPosts.map((post, idx) => (
        <div key={post._id} className="group flex w-full flex-col gap-4 md:flex-row" data-post-id={post._id}>
              ...
              {post.authors.length > 1 ? (
                <div className="flex items-center gap-2">
                  {post.authors.map((author) => (
                    <div key={author._id} className="flex items-center gap-2">
                      ...
                      />
                      <p className="text-sm text-(--text-tertiary) dark:text-(--dark-text-tertiary)">{author._title}</p>
                    </div>
                  ))}
                </div>
              ) : post.authors[0] ? (
                <div className="flex items-center gap-2 rounded-full">
                  <img
                    ...
                    height={24}
                    src={post.authors[0].image.url || "/placeholder.svg"}
                    width={24}
                    className="rounded-full"
                  />
