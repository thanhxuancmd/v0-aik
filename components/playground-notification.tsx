...
export function PlaygroundSetupModal({ playgroundInfo, envs }: PlaygroundSetupModalProps) {
  ...
  const formatTimeRemaining = (expiresAt: string | null) => {
    ...
    if (days > 0) {
      return `${days} ngày${days !== 1 ? "s" : ""}, ${hours} giờ${hours !== 1 ? "s" : ""}`
    }
    return `${hours} giờ${hours !== 1 ? "s" : ""}`
  }
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[999] flex items-start justify-end p-4">
      ...
      <div
        className={cn(
          "border-1 pointer-events-auto absolute right-4 top-4 flex w-[500px] origin-top flex-col rounded-xl border-solid border-[--border] bg-[--surface-primary] shadow-xl outline-none transition-all duration-200",
      ...
      >
        <div className="flex flex-shrink-0 items-center justify-between rounded-t-xl border-b border-dashed border-[--border] bg-[--surface-secondary] px-4 py-3">
          <div className="flex items-center gap-2">
            <span className={cn("text-base font-semibold text-[--text-primary]")}>Thiết lập Website</span>
          </div>
          <a
          ...
          >
            <ExternalLink className="h-4 w-4" /> Mở Playground
          </a>
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="mb-1 text-sm font-medium text-[--text-primary]">
                      Playground này sẽ hết hạn trong{" "}
                ...
                {envResults.map((env, index) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="truncate font-mono text-xs text-[--text-tertiary]">{env.name}</div>
                    </div>
                    <div
                      className={cn(
                        "text-xs font-medium",
                    ...
                    >
                      {env.isValid ? "Đã thiết lập" : "Thiếu"}
                    </div>
                  </div>
                ))}
              </div>
