...
export function ThemeProvider({ theme }: { theme: Theme }) {
  ...
  const css = Object.entries(accent).map(([key, value]) => {
    ...
    const result = [`--accent-${key}: ${value}; --accent-rgb-${key}: ${rgb.r}, ${rgb.g}, ${rgb.b};`]
  ...
  Object.entries(grayScale).forEach(([key, value]) => {
    const rgb = anyColorToRgb(value)
    const result = [`--grayscale-${key}: ${value}; --grayscale-rgb-${key}: ${rgb.r}, ${rgb.g}, ${rgb.b};`]
